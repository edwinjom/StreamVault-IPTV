    ...
    Box(modifier = modifier) {
        Surface(
            ...
            modifier = Modifier
                ...
                .mouseClickable(focusRequester = toggleRequester, onClick = { showSourceList = !showSourceList })
            ...
            Row(
                ...
                Text(
                    text = currentOption?.title ?: noSourceLabel,
                    style = if (compact) MaterialTheme.typography.labelMedium else MaterialTheme.typography.bodyMedium,
                    modifier = Modifier.weight(1f).widthIn(max = if (compact) 180.dp else 280.dp),
                    color = if (isFocused) Primary else OnBackground
                )
                Icon(imageVector = if (showSourceList) Icons.Default.KeyboardArrowUp else Icons.Default.KeyboardArrowDown, contentDescription = null, tint = OnSurfaceDim)
            }
        }

        if (showSourceList && options.isNotEmpty()) {
            Surface(
                modifier = Modifier.padding(top = if (compact) 38.dp else 46.dp).width(dropdownWidth),
                shape = RoundedCornerShape(12.dp),
                ...
                Column(modifier = Modifier.padding(8.dp)) {
                    options.forEach { option ->
                        LiveSourceItem(
                            ...
                            onClick = { onSourceSelected(option.source); showSourceList = false },
...
private fun LiveSourceItem(
    ...
    Surface(
        onClick = onClick,
        modifier = Modifier.fillMaxWidth().focusRequester(requester).mouseClickable(focusRequester = requester, onClick = onClick).onFocusChanged { isFocused = it.isFocused },
        ...
        Row(modifier = Modifier.padding(horizontal = 12.dp, vertical = 8.dp), horizontalArrangement = Arrangement.SpaceBetween, verticalAlignment = Alignment.CenterVertically) {
            Column(modifier = Modifier.weight(1f)) {
                Text(text = option.title, style = MaterialTheme.typography.bodyMedium, color = if (isFocused || isSelected) Primary else OnBackground)
                option.subtitle?.takeIf { it.isNotBlank() }?.let { Text(text = it, style = MaterialTheme.typography.labelSmall, color = OnSurfaceDim) }
            }
            Text(text = when { isSelected -> selectedLabel; !option.isEnabled -> unavailableLabel; else -> "" }, style = MaterialTheme.typography.labelSmall, color = if (isSelected) Primary else OnSurfaceDim)
        }
    }
}
