package com.streamvault.feature.system.test
...
fun SemanticsNodeInteraction.assertAgainstGolden(goldenName: String) {
    val bitmap = captureWithRetry(goldenName)
        ...
        instrumentation.uiAutomation
            .executeShellCommand(
                "cp ${outputDirs.last().absolutePath}/$goldenName.png " +
                    "/sdcard/Download/streamvault-system-golden-$goldenName.png"
            )
            .close()
        return
    }
    ...
    val expected = try {
        ...
        throw AssertionError(
            "Missing golden asset ui-goldens/$goldenName.png; " +
                "run the connected test with -PsystemGoldens.record=true and copy the " +
                "generated file into feature/system/src/androidTest/assets/ui-goldens/"
        )
    }
    assertBitmapsEqual(expected, bitmap, goldenName)
}

private fun SemanticsNodeInteraction.captureWithRetry(goldenName: String): Bitmap {
    ...
    throw AssertionError(
        "Failed to capture golden $goldenName after $CAPTURE_ATTEMPTS attempts",
        lastFailure,
    )
}
